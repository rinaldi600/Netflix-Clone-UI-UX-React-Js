...
export default function Adventure(props) {
    ...
    useEffect(() => {
        if (count > 200) {
            setCount(0);
        } else if (window.innerWidth < 576 && count < -1800) {
            setCount(-200)
        } else if (window.innerWidth > 1200 && count < -800) {
            setCount(-200)
        } else if (window.innerWidth > 577 && count < -1400) {
            setCount(-200)
        }
    });
    ...
    return (
        <div className={"flex bg-black overflow-hidden relative"}>
            ...
            <div style={move} className={`bg-black grid h-96 ${listMovies.length === 0 ? 'w-full justify-content-center p-2 content-center' : 'grid-flow-col gap-7 content-center p-2' }`}>
                {
                    listMovies.length === 0 ?
